...
import com.duy.casiofx.factory.VectorFunctionFactory;
import com.duy.casiofx.factory.VectorOperatorFactory;
...
import com.duy.casiofx.token.StringToken;
...
//                if (bracket.getType() == BracketToken.OPEN || bracket.getType() == BracketToken.SUPERSCRIPT_OPEN
...
//                    stack.push(bracket);
//                } else if (bracket.getType() == BracketToken.CLOSE || bracket.getType() == BracketToken.SUPERSCRIPT_CLOSE
    ...
    public static Token evaluateExpression(ArrayList<Token> tokens) {
        ...
        for (Token token : tokens) {
            if (token instanceof NumberToken || token instanceof VectorToken) { //Adds all numbers directly to the stack
                ...
                stack.push(new NumberToken(((OperatorToken) token).operate(((NumberToken) left).getValue(), ((NumberToken) right).getValue()))); //Adds the result back to the stack
                ...
                stack.push(new NumberToken(((FunctionToken) token).perform(((NumberToken) top).getValue()))); //Adds the result back to the stack
