...
public class GraphActivity extends AbstractAppCompatActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        imgTrace = (Switch) findViewById(R.id.img_trace);
        imgDervi = (Switch) findViewById(R.id.btn_der);
        mModeGraph = (Switch) findViewById(R.id.sw_mode);
        ...
        if (!(mPreferences.getBoolean(GRAPH_STATED, false)) || ConfigApp.DEBUG) showHelp();
    ...
    private void saveImage() {
        try {
            ...
            if (root.canWrite()) {
                Date d = new Date();
                File imageLoc = new File(root, "/ncalc/" + System.currentTimeMillis() + ".png");
    ...
    private void changeModeDerivative(boolean isDerivative) {
        ...
        if (!result) {
            ...
            Toast.makeText(GraphActivity.this, getString(R.string.noFunDisp), Toast.LENGTH_LONG).show();
        } else if (this.isDerivative) {
            Toast.makeText(GraphActivity.this, getString(R.string.tapFun), Toast.LENGTH_LONG).show();
        }
    }
    ...
    private void receiveData() {
        ...
        if (bundle != null) {
            ...
            if (fx != null && !fx.isEmpty()) mPreferences.edit().putString("f1", fx).apply();
        }
    }
