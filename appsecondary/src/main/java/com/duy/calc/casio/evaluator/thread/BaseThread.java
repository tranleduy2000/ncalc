...
public abstract class BaseThread {
    ...
    public abstract void execute(List<Token> tokens, EvaluateConfig config);

    public ArrayList<Token> subAns(List<Token> tokens) {
        ArrayList<Token> function = new ArrayList<>();
        for (Token token : tokens) {
            if (token instanceof VariableToken && token.getType() == VariableToken.ANS) {
                for (Token t : VariableFactory.ansValueFunc) {
                    if (t instanceof VariableToken && t.getType() == VariableToken.CONSTANT) {
                        function.add(new NumberToken(0));
                    } else {
                        function.add(t);
                    }
                }
    ...
    public interface ResultCallback {
        void onSuccess(ArrayList<Token> result);
